...
class OnDelete(str, Enum):
    ...
    def operation_handler(
        ...
        if isinstance(referencing_model, str):
            try:
                referencing_model = import_string(referencing_model)
        ...
        referring_objects = referencing_model.filter(**{filter_key: obj_id})

        if not referring_objects:
            return
