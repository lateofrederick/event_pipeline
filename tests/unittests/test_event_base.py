...
class TestEventBaseAdditional(unittest.TestCase):
    ...
    def test_evaluator_raises_if_strategy_is_none(self):
        with pytest.raises(ImproperlyConfigured, match="No result evaluation strategy specified"):
            self.NoStrategyEvent.evaluator()

    def test_evaluator_raises_if_strategy_is_invalid(self):
        with pytest.raises(ImproperlyConfigured, match="is not a valid result evaluation strategy"):
    ...
    def test_get_version_handler_is_cached(self):
        ...
        with patch("volnux.base.VersionHandler.from_class", return_value=fake_handler) as mocked:
        ...
        mocked.assert_called_once_with(self.SimpleEvent, config_key="DEFAULT_EVENT_VERSIONING")
    ...
    def test_get_version_info_delegates_to_handler(self):
        ...
        with patch.object(self.SimpleEvent, "get_version_handler", return_value=fake_handler):
    ...
    def test_get_all_versions_uses_registry_with_handler_metadata(self):
        ...
        with patch.object(self.NamedEvent, "get_version_handler", return_value=fake_handler), patch(
